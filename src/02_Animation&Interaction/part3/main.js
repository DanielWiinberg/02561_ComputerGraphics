...
window.onload = function init() {
    ...
    gl.bufferData(
        gl.ARRAY_BUFFER,
        6 * maxVertices * sizeof["vec2"],
        gl.STATIC_DRAW
    ); //Pre-allocate space for buffer
    ...
    // COLORS
    const colorMenu = document.getElementById("color-selection");
    ...
    // DRAWING MODE
  const drawMode = document.getElementById("drawing-mode");
...
function drawPoint(event) {
  ...
  const offset = 0.01;
  const pointTriangle = [ 
    vec2(pos[0] - offset, pos[1] - offset),
    vec2(pos[0] + offset, pos[1] - offset),
    vec2(pos[0] - offset, pos[1] + offset),
    vec2(pos[0] - offset, pos[1] + offset),
    vec2(pos[0] + offset, pos[1] - offset),
  ...
  const triangleColor = [color, color, color, color, color, color];
...
function findPointClicked(event) {
